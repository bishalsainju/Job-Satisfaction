...
file_path = "~/Desktop/R/data/company_data/2/1_50_1000_pros.csv"
...
poliblogPrevFit <- stm(out$documents, out$vocab, K=10, prevalence=~Ratings+Job_Status, 
...
predict_topics <- estimateEffect(formula = 1:10 ~ Ratings + Job_Status + s(Reviewed_Year), 
...
plot(predict_topics, covariate = "Ratings", topics = predict_topics$topics,
     ...
     main="Effect of Ratings(Positive Feedback)",
...
plot(poliblogPrevFit, type="perspectives", topics=c(3,5))
...
# help(stmCorrViz)
# findThreshold(poliblogPrevFit, documents_raw=processed$documents, 
#               documents_matrix=out$documents, 
#               range_min=.0001, range_max=.001, step=.0005)
# stmCorrViz(poliblogPrevFit, "~/Desktop/R/img/1/corr_tree.html", 
#            processed$documents, out$documents, title = "Hierarchical Clustering",
#            clustering_threshold=5, labels_number=3, display=TRUE,
#            verbose=TRUE)
